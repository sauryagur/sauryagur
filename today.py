...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    if request.status_code == 200:
        if request.json()['data']['repository']['defaultBranchRef'] != None:  # Only count commits if repo isn't empty
            ...
            return 0, 0, 0  # Fixed: return tuple instead of single value
...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    ...
    for node in history['edges']:
        # Fixed: Check if author exists and has user
        if (node['node']['author'] and
                node['node']['author']['user'] and
                node['node']['author']['user']['id'] == OWNER_ID):
            my_commits += 1
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    filename = 'cache/' + hashlib.sha256(
        USER_NAME.encode('utf-8')).hexdigest() + '.txt'  # Create a unique filename for each user
    ...
    if len(data) - comment_size != len(
            edges) or force_cache:  # If the number of repos has changed, or force_cache is True
        cached = False
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            ...
            except (TypeError, AttributeError):  # If the repo is empty or defaultBranchRef is None
...
def add_archive():
    ...
    try:
        ...
        # If archive file doesn't exist, return zeros
        return [0, 0, 0, 0, 0]
    ...
    # Fixed: Handle potential index error
...
def user_getter(username):
    ...
    return request.json()['data']['user']['id'], request.json()['data']['user'][
        'createdAt']  # Fixed: return as tuple, not dict
...
if __name__ == '__main__':
    try:
        ...
        user_id, acc_date = user_getter(USER_NAME)  # Fixed: unpack tuple correctly
        OWNER_ID = user_id
        ...
        if OWNER_ID == 'MDQ6VXNlcjU3MzMxMTM0':  # only calculate for user Andrew6rant - Fixed: compare string directly
